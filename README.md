...
### Summary
Farmy & Foods, a company in the agricultural sector that produces and harvests different types of food is facing a challenge where their cherry plantations have been presenting powdery mildew, which is a fungal disease that affects a wide range of plants. 
The cherry plantation crop is one of their finest products and the company is concerned about supplying the market with a product of compromised quality.
This is a web app designed to help users detect powdery-mildew on cherry tree leaves using a machine learning model to visually differentiate between infected and healthy leaves, thus making the task of checking many trees more expedient and efficient.


### Deployed version at [mildew-detection-in-cherry01-b61ac18ef016.herokuapp.com/](https://mildew-detection-in-cherry01-b61ac18ef016.herokuapp.com/)
...
The images are taken from the client's crop fields and show leaves that are either healthy or infested by powdery mildew.
The dataset used to train this model contains 4208 featured photos of single cherry leaves against a neutral background.
- More on [powdery mildew](https://treefruit.wsu.edu/crop-protection/disease-management/cherry-powdery-mildew/)
- The dataset source [Kaggle](https://www.kaggle.com/datasets/codeinstitute/cherry-leaves).

## Business Requirements

Farmy & Food requested to develop a Machine Learning based system to detect whether a certain cherry tree presents powdery mildew so it can be treated with a fungicide. The requested system should be capable of detecting infection on a tree by using a tree leaf image, to determine if it needs attention. The system was requested to automate the detection process conducted manually thus far. The company has thousands of cherry trees, located on multiple farms across the country. As a result, this manual process is not scalable due to the time spent in the manual process inspection.

Link to the full [business interview].(https://github.com/BlackBoltBeta/Mildew_Detection_in_Cherry_Leaves.wiki.git)
...
**Hypothesis 1**: Infected leaves have clear marks differentiating them from the healthy leaves.
   - **How to validate**: An average image study can help to corroborate if this  is correct.

**Hypothesis 2**: Mathematical formulas comparison: `softmax` performs better than `sigmoid` as activation function for the CNN output layer. 
...
![montage_healthy](assets/readme_images/healthy_montage.png)
![montage_infected](assets/readme_images/powdery_mildew_montage.png)

Difference between average and variability images shows that affected leaves present more white stipes on the center.

![average variability between samples](assets/readme_images/average_infected.png)

While image difference between average infected and average infected leaves shows no intuitive difference.

![average variability between samples](assets/readme_images/average_healthy.png)

**Conclusion**

The model was able to detect the differences between leaves which were infected with powdery-mildew and those without the infection, and was also able to learn how to differentiate and generalize in order to make accurate predictions.
A good model trains its ability to predict classes on a batch of data without adhering too closely to that set of data.
In this way, the model is able to generalize and predict future observation reliably because it didn't 'memorize' the relationships between features and labels as seen in the training dataset, but rather the general pattern from feature to labels.
...
**Introduction**
   ...
   1. Establish the  problem and mathematical functions to solve it.

The model is required to assign to a cherry leaf one of the two categories: healthy or infected, this makes it a classification problem.
This problem could be seen as a binary classification healthy vs infected or as a multi-class classification where each output is assigned one and only one label from more than two classes, just two in our case: healthy vs infected.
...
The model has 1 input layer, hidden layers (2 ConvLayer, 1 Flatten Layer and 1 FullyConnected), 1 output layer. 
...
The model does not necessarily represent the best one but this structure was eventually chosen evaluating the outcome of multiple tests and tuning the model according to the goal.
...
- **Optimizer**: An optimizer is a function or algorithm that is created and used for neural network attribute modification (i.e., weights, learning rates) for the purpose of speeding up convergence while minimizing loss and maximizing accuracy. In this case, `adam` was set as the optimizer. It is a popular optimization algorithm in deep learning. It combines ideas from RMSprop and Momentum, providing adaptive learning rates and efficient optimization, `adam` is commonly used for a wide range of tasks due to its effectiveness in practice.

- **Metrics**: `accuracy` Calculates how often predictions equal labels. This metric creates two local variables, total and count that are used to compute the frequency with which `y_pred` matches `y_true`. In this way accuracy is calculated as the ratio of correctly predicted instances to the total number of instances. It provides a straightforward measure of how well the model is performing in terms of correct classifications.
...
## Trial and error
Part of the process that lead to the current hyperparameters settings and model architecture is mostly trial and error, changing one parameter at a time while.
...
In terms of **User Story**:
- As a client I want a ML model to predict with an accuracy of at least 97% whether a given cherry leaf is healthy or contains powdery mildew. 
...
**Visit the Project Handbook on [Wiki](https://github.com/BlackBoltBeta/Mildew_Detection_in_Cherry_Leaves.wiki.git)**
...
![leaf_detector](assets/readme_images/detector_screenshot.png)
...
- tensorflow-cpu  used for creating the model
- numpy           used for converting to array 
- scikit-learn    used for evaluating the model
- streamlit       used for creating the dashboard
- pandas          used for creating/saving as dataframe
- matplotlib      used for plotting the sets' distribution
- keras           used for setting model's hyperparamters
- plotly          used for plotting the model's learning curve 
- seaborn         used for plotting the model's confusion matrix
