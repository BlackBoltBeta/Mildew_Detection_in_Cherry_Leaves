...
def page_leaves_visualizer_body():
    ...
    st.info(
        f"A study that visually differentiates a cherry"
        f" leaf affected by powdery mildew from a healthy one.")
    ...
    st.warning(
        f"We suspect cherry leaves affected"
        f" by powdery mildew have clear marks,"
        f" typically the first symptom is a light-green,"
        f" circular lesion on either leaf surface,"
        f" then a subtle white cotton-like growth"
        f" develops in the infected area.\n\n"
        f" This property has to be translated in machine learning terms,"
        f" images have to be 'prepared' before"
        f" being fed to the model for an optimal"
        f" feature extraction and training.\n\n"
        f" When we are dealing with an Image dataset,"
        f" it's important to normalize the images in the"
        f" dataset before training a Neural Network on it."
        f" To normalize an image, one will need the mean"
        f" and standard deviation of the entire"
        f" dataset that are calculated with a mathematical formula"
    ...
    if st.checkbox("Difference between average and variability image"):
        ...
        st.warning(
            ...
            f" patterns where we could intuitively"
            f" differentiate one from another."
            f" However, mildew affected leaves show"
            f" more white stipes on the center.")
